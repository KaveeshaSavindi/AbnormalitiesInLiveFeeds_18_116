...
namespace POC
{
    public partial class Form1 : Form
    {
        ...
        MCvObjectDetection[] bodies;// DETECTED BODIES
        ...
        {            
            try
            {
                ...
                // Detecting bodies
                if (bodyDetection) { 
                    bodies=descriptor.DetectMultiScale(Gray_Frame,useMeanshiftGrouping:true);
                    foreach (MCvObjectDetection body in bodies)
                        {
                            img.Draw(body.Rect, new Bgr(System.Drawing.Color.Red), 2);
                        }
                    lblBodies.BeginInvoke((Action)(()=>{// This is used to update the label of Faces detected count
                        lblBodies.Text = bodies.Length + "";
                    }));
                }
                //
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            taCAMERA.deleteByName("ATM");
            taCAMERA.Insert(EqualizeHist + "", GammaCorrect + "", GaussianBlure + "", bodyDetection + "", detectMotionAnoramlities + "", "False", freezDelay + "", "20", null, "ATM");
        }
